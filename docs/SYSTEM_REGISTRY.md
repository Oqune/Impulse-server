...
### 3.1. Сетевая архитектура: IP-Agnostic Reachability vs LAN-суверенность

> **Важнейший тезис для защиты:** «Импульс» — это **полноценное IP-агностичное приложение**, способное работать поверх **любой** сетевой инфраструктуры. Обозначение «LAN» подчеркивает не техническое ограничение, а **гарантию суверенности (Zero External Dependencies)** в условиях изоляции.

* **Работа через глобальный Интернет (WAN / Internet):**
  Сервер может быть развернут на любом VDS/VPS хостинге или домашнем сервере с белым IP/доменом. Клиенты подключаются отовсюду через WebTransport (QUIC/UDP 4433).
* **Работа через VPN / оверлейные сети (Tailscale, WireGuard, ZeroTier):**
  Связывает распределенные узлы в защищенный закрытый контур по внутренним IP-адресам.
* **Работа в изолированной локальной сети (Sovereign LAN):**
  Хотспот смартфона, домашний роутер, полевой коммутатор без выхода в глобальный интернет.
* **Главное отличие от аналогов (Telegram, Signal):**
  Обычные мессенджеры привязаны к облачной инфраструктуре третьих лиц (серверы в США/Европе, DNS, SMS-шлюзы, Apple/Google Push). При падении внешнего интернета они мгновенно выходят из строя. «Импульсу» для работы нужен **исключительно IP-адрес и порт 4433** хоста — он не требует внешних сервисов, но масштабируется на весь глобальный Интернет.
